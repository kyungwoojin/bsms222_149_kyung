...
![firstimage](https://ars.els-cdn.com/content/image/1-s2.0-S1525157810603546-gr2.gif)
...
```{r}
# 40 variants are occurred in muliple times
length(which(c$Freq>1))
# I would like to use c.DNA, Pos_hg19 
# recurrent mutation in c.DNA = 40
# recurrent mutation in Pos_hg19 = 44
c_c.DNA<-as.data.frame(table(d$c.DNA))
length(which(c_c.DNA$Freq>1))
c_Pos_hg19<-as.data.frame(table(d$Pos_hg19))
length(which(c_Pos_hg19$Freq>1))
```
...
```{r}
library(tidyverse)
c_clf<-as.data.frame(table(d$Classification))%>%mutate(total=Freq/293*100)
c_clf
```

Then, you might be intrigued to whether females and males have different occurrence in each disorder. Let's check it. 
```{r}
c_Clf_sex<-filter(d,PatientSex=="M"|PatientSex=="F")
as.data.frame(table(Classification=c_Clf_sex$Classification,Sex=c_Clf_sex$PatientSex))%>%arrange(Classification)
```
...
```{r}
as.data.frame(table(phenotype=d$Classification,mutation=d$Effect))%>%arrange(phenotype)
```
...
library(tidyverse)
d1<-d
d1<-mutate( d1, loci=b<-gsub('[^0-9]','',c.DNA))%>%filter( c.DNA!='.')
d2<-as.data.frame(table(loci=d1$loci,conseqence=d1$Effect))%>%filter(Freq>1)%>%arrange(-Freq)
d2
```
