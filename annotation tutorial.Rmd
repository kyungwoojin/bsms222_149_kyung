...
read Png(https://media.springernature.com/full/springer-static/image/art%3A10.1186%2F1471-2164-16-S8-S2/MediaObjects/12864_2015_Article_7216_Fig1_HTML.jpg?as=webp)

**Figure 1. Comparison of GENCODE and RefSeq gene annotation and the impact of reference geneset on variant effect prediction** (Frankish et al. 2015)
...
```{r}
# 8 feature types are in the dataset.
```
...
Havana: Manual gene annotation (detailed strategy in here)
Ensembl: Automatic gene annotation (detailed strategy in here)
...
```{r}
d_copy%>%filter(type=="transcript")%>%count(gene_name)%>%summarize(mean(n), quantile(n,0.25),quantile(n,0.5),quantile(n,0.75),gene_name[which.max(n)])
```
...
```{r}
d_copy%>%filter(type=="transcript")%>%group_by(gene_type)%>%count(gene_name)
```

3. Final task is to compute the number of transcripts per gene per chromosome.
```{r}
d_copy%>%filter(type=="transcript")%>%group_by(seqnames)%>%count(gene_name)
```

## 3.2. Gene length in the GENCODE
1. What is the average length of human genes?
```{r}
d_copy%>%filter(type=="gene")%>%summarize(average=mean(width))
```

2. Is the distribution of gene length differed by autosomal and sex chromosomes? 
Please calculate the quantiles (0%, 25%, 50%, 75%, 100%) of the gene length for each group.
```{r}
d_copy%>%filter(type=="gene")%>%select(seqnames, width)%>%filter(seqnames!="chrM")%>%mutate(chr_type=ifelse(seqnames%in%c("chrX", "chrY"), "sex", "auto"))%>%summarize(auto_quantile=quantile(width[chr_type=="auto"], c(0, 0.25, 0.5, 0.75, 1)), sex_quantile=quantile(width[chr_type=="sex"], c(0, 0.25, 0.5, 0.75, 1)))
```
...
```{r}
d_copy%>%filter(type=="gene")%>%select(gene_type, width)%>% summarize(gene_length_quantile=quantile(width, c(0, 0.25, 0.5, 0.75, 1)))
```

## 3.3. Transcript support levels (TSL)
1. The GENCODE TSL provides a consistent method of evaluating the level of support that a GENCODE transcript annotation is actually expressed in humans.
With transcript, how many transcripts are categorized for each TSL?
```{r}
d_copy%>% filter(type=="transcript")%>% filter(transcript_support_level!="NA")%>%nrow()
```

2. From the first question, please count the number of transcript for each TSL by gene biotype.
```{r}
d_copy%>% filter(type=="transcript")%>% filter(transcript_support_level!="NA")%>%count(transcript_support_level, gene_type)
```

3. From the first question, please count the number of transcript for each TSL by source.
```{r}
d_copy%>% filter(type=="transcript")%>% filter(transcript_support_level!="NA")%>%count(transcript_support_level, source)
```
...
```{r}
d3<-d_copy%>%filter(type=="gene")%>%select(gene_id, gene_name, hgnc_id, gene_type, seqnames, start, end, strand)%>%mutate(isHgnc=ifelse(is.na(hgnc_id),"0","1"), isCCDS=ifelse(is.na(d_copy$ccdsid[d_copy$type=="gene"]),"0","1"))
```

2. Please count the number of hgnc by gene biotypes.
```{r}
d3%>%filter(isHgnc==1)%>%select(hgnc_id, gene_type)%>%count(gene_type)
```
...
```{r}
d_copy%>%filter(type=="gene")%>%select(level, hgnc_id)%>%filter(hgnc_id!="NA")%>%count(level)
```

## 3.5. Transcripts in the GENCODE
1. Which gene has the largest number of transcripts?
```{r}
d_copy%>%filter(type=="transcript")%>%count(gene_name)%>%summarize(gene_name[which.max(n)])
```

2. Please calculate the quantiles (0%, 25%, 50%, 75%, 100%) of the gene length for protein coding genes and long noncoding genes.
```{r}
d_copy%>%filter(type=="gene")%>%summarize(protein_coding_genes=quantile(width[gene_type=="protein_coding"], c(0,0.25,0.5,0.75,1)), long_noncoding_genes= quantile(width[gene_type=="lncRNA"], c(0,0.25,0.5,0.75,1)))
```

3. Please count the number of transcripts by chromosomes.
```{r}
d_copy%>%filter(type=="transcript")%>%count(seqnames)
```

## 3.6. Autosomal vs. Sex chromosomes.
1. Please calculate the number of genes per chromosome.
```{r}
d_copy%>%filter(type=="gene")%>%count(seqnames)
```

2. Please compare the number of genes between autosomal and sex chromosome (Mean, Median).
```{r}
d_copy%>%filter(type=="gene")%>%filter(seqnames!="chrM")%>%mutate(chr_type=ifelse(seqnames%in%c("chrX", "chrY"), "sex", "auto"))%>%count(chr_type)
```
...
```{r}
d_copy%>%filter(type=="gene"& gene_type%in%c("protein_coding", "lncRNA"))%>%count(seqnames, gene_type)
```
