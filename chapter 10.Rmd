...
![firstimage](https://rafalab.github.io/dsbook/book_files/figure-html/piechart-1.png)
...
![secondimage](https://rafalab.github.io/dsbook/book_files/figure-html/donutchart-1.png)
...
![thirdimage](https://rafalab.github.io/dsbook/book_files/figure-html/two-barplots-1.png)
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/common-axes-histograms-right-2-1.png)
...
```{r}
dat %>% mutate(state = reorder(state, rate, FUN = mean)) %>% ggplot(aes(state, rate)) + geom_bar(stat="identity") + coord_flip()
```
...
```{r}
sp <- us_contagious_diseases %>%
  filter(!state %in% c("Hawaii", "Alaska") & disease == "Smallpox" & weeks_reporting>=10 ) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate))
sp %>% ggplot(aes(year, state, fill = rate)) + geom_tile(color = "grey50") +scale_x_continuous(expand=c(0,0)) +
  scale_fill_gradientn(colors = brewer.pal(9, "Reds"), trans = "sqrt") +  theme_minimal() +  theme(panel.grid = element_blank(), 
        legend.position="bottom", 
        text = element_text(size = 8)) +
  ggtitle("Smallpox") + 
  ylab("") + xlab("")
...
```{r}
sp2 <- us_contagious_diseases %>%
  filter(disease == "Smallpox" & weeks_reporting>=10) %>% group_by(year) %>%  summarise(us_rate=sum(count,na.rm=TRUE)/sum(population,na.rm=TRUE)*10000) 
sp %>% 
  filter(!is.na(rate)) %>%
    ggplot() +
  geom_line(aes(year, rate, group = state),  color = "grey50", 
            show.legend = FALSE, alpha = 0.2, size = 1) +
  geom_line(mapping = aes(year, us_rate),  data =sp2 , size = 1) +
...
```{r}
cal <- us_contagious_diseases %>%   filter(state == "California" & weeks_reporting>=10) %>%  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate))
cal %>% ggplot() + geom_line(aes(year, rate, col=disease), size=1) + scale_y_continuous(trans="sqrt", breaks=c(5,25,125)) + ggtitle("Cases per 10,000 in California") +   xlab("") + ylab("")
```
...
```{r}
  US <- us_contagious_diseases %>%   filter(weeks_reporting>=10) %>%  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate))
US %>% ggplot() + geom_line(aes(year, rate, col=disease), size=1) + scale_y_continuous(trans="sqrt", breaks=c(5,25,125,300)) + ggtitle("Cases per 10,000 in US") +   xlab("") + ylab("")
```
