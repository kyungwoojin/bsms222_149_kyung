...
We say that a data table is in tidy format if each row represents one observation and columns represent the different variables available for each of these observations. The ```murders```
...
```{r}
murders <- mutate(murders, rate= murder_rate/100000)
```
