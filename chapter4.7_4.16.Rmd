...
NHANES %>% filter(AgeDecade== ' 20-29')%>% summarize(min=min(BPSysAve, na.rm=TRUE), max=max(BPSysAve, na.rm=TRUE))
```
...
NHANES%>% group_by(Race1)%>%filter(AgeDecade==' 40-49',Gender=='male')%>%
  summarize(average=mean(BPSysAve, na.rm=TRUE))%>%arrange(average)
```
...
```{r}
compute_s_n_2 <- function(c){
  k=1:c
    tibble(n=c,s_n=sum(k), s_n_2= sum(k))
}
map_df(n, compute_s_n_2)
```
