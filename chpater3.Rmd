...
```
In each iteration $n=1$, $n=2$, etc..., we compute $S_{n}$ nd store it in the $n$th entry of ```s_n```.
...
```{r}
new_names<- murders$abb[which(nchar(murders$state)>8)]
new_names
```
...
## 3
```
7. Write a function ```compute_s_n``` that for any given computes the sum $S_n=1^2+2^2+3^2+...n^2$Report the value of the sum when ```n= 10```.
```{r}
